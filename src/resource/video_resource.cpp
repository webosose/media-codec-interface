...
bool VideoResource::Reacquire(DeviceType device_type,
  ...
  if (!requestor_->ReacquireResources(resourceMMap,
           source_info, resources)) {
    MCIL_ERROR_PRINT("Resource acquisition failed");
    return false;
  }
  ...
  {
	MCIL_DEBUG_PRINT("Resource acquisition success");
  }
  ...
  if (*vdec_index < 0)
  {
    MCIL_ERROR_PRINT(" failed acquiring and *vdec_index is: %d", *vdec_index);
    return false;
  }
